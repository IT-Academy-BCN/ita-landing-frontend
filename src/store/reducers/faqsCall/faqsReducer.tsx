...
export const { setFaqs } = apiSlice.actions;
 ...
 export const apiCall =async(dispatch:Dispatch)=>{
    try {
        const faqsCalled = await axios.get('http://http://87.106.229.119/api/faqs');
  ...
  //Post //
 export const postApiFaqs =async(faqContent:faqContent,acces_token:string, dispatch:Dispatch)=>{
    try{
       const newFaq= {
      title:faqContent.title,
      description:faqContent.description
    }
     await axios.post('http://http://87.106.229.119/api/faqs',newFaq,{
      headers:{
  ...
  export const deleteApiFaqs = async (faqId:number, acces_token:string,dispatch:Dispatch) => {
    try {
      const url = `http://http://87.106.229.119/api/faqs/${faqId}`;
  ...
  export const putApiFaqs =async(faqId:number, faqContent:any, acces_token:string, dispatch:any)=>{
    try{
      await axios.put(`http://http://87.106.229.119/api/faqs/${faqId}`,faqContent,{
        headers:{
          Authorization:`Bearer ${acces_token}`
        }
      });  
