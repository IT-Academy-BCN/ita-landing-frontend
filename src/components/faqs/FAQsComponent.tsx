...
import {
  apiCall,
  deleteApiFaqs,
} from "../../store/reducers/faqsCall/faqsReducer";
import { createToken, Faq } from "../../interfaces/interfaces";
...
const FAQs = () => {
  ...
  const { faqs }: any = useSelector((state: RootState) => state.faqsReducer);
            ...
            {window.location.pathname == "/backoffice" && (
              <div className="flex justify-end mr-20 pb-7">
                <button
                  ...
                  onClick={() => deleteApiFaqs(faq.id, acces_token, dispatch)}
                >
                  Eliminar
                </button>
                <button className="btn xl:px-9  bg-pink-it text-white">
                  Guardar
                </button>
              </div>
            )}
